...
{
      var iframe = document.getElementById('bguFrame');
  
      // d = document.createElement('iframe');
      d=document.getElementById('dFrame')
      d.style.width = iframe.offsetWidth + 'px';
        ...
        d.id = 'd';
        d.tagName = 'd'; 
      	//document.getElementById("splitinterface-widget-container").appendChild(d)
	...
	//			$("#btnContinue").html('Continue')
	    ...
	    case 9:
	    	...
	    	for (var i = votes.length - 1; i >= 0; i--) {
	    		if( votes[i].checked)
	    			voting=votes[i].value
	    	};
	        if(voting=="NA")
	        {
	        	alert("Please Vote for one option")
	        	onContinue.curPage--
	        }
			if (onContinue.curPage==9)
			{
				rateA=$("#range_sol").val()
	        	servlog("conf.Sol",rateA)
	         	rateB=$("#range_vote").val()
	        	servlog("conf.Vote",rateB)
				servlog("vote",voting)
				var explanationVote=$("#explanationVote").val()
